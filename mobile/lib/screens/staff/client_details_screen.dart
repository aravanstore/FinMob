...
class _ClientDetailsScreenState extends State<ClientDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF0F172A),
        title: const Text('Профиль клиента', style: TextStyle(color: Colors.white)),
        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            ...
            children: [
              ...
              // Сводка
              const Text('Сводка по кредитам', style: TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.bold)),
              ...
              // Список кредитов
              const Text('Кредиты', style: TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              if (loans.isEmpty)
                const Center(child: Text('Кредитов не найдено', style: TextStyle(color: Colors.white38)))
              else
  ...
  Widget _loanTile(dynamic l, NumberFormat fmt) {
    ...
    final color = status == 'Просрочен' ? Colors.redAccent : (status == 'Активен' ? Colors.blue : Colors.green);
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('№${l['contract_number']}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              Text(status, style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 8),
          Text('${fmt.format(double.parse(l['principal_balance'].toString()))} сом',
              style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
          const Text('Остаток основного долга', style: TextStyle(color: Colors.white38, fontSize: 11)),
        ],
      ),
    );
  }
