...
class ShareDetailsScreen extends StatefulWidget {
  ...
  const ShareDetailsScreen({super.key, required this.clientId, required this.clientName});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: pal.bg,
        elevation: 0,
        title: Text('История паёв', style: TextStyle(color: pal.textPri, fontSize: 18, fontWeight: FontWeight.bold)),
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(color: pal.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: pal.border)),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Заёмщик', style: TextStyle(color: pal.textSec, fontSize: 11)),
                const SizedBox(height: 4),
                Text(widget.clientName, style: TextStyle(color: pal.textPri, fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          ...
          Expanded(
            child: FutureBuilder<List<dynamic>>(
              ...
              builder: (context, snap) {
                ...
                if (list.isEmpty) {
                  return Center(child: Text('Операций по паям еще не было', style: TextStyle(color: pal.textSec)));
                }
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(label, style: TextStyle(color: pal.textPri, fontWeight: FontWeight.w600, fontSize: 14)),
                                const SizedBox(height: 4),
                          ...
                          Text(
                            ...
                            style: TextStyle(color: amount > 0 ? _C.green : pal.textPri, fontWeight: FontWeight.bold, fontSize: 15),
                          ),
                        ],
                      ),
                    );
